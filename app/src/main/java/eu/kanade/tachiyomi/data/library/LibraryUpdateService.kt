 ...
 * progress of the update, and in case of an unexpected error, this service will be silently
 * destroyed.
 */
class LibraryUpdateService(
    ...
    override fun onDestroy() {
        // TX-->
        preferences.libraryUpdateEndTIme().set(System.currentTimeMillis())
    ...
    fun addMangaToQueue(categoryId: Int) {
        ...
        // TX-->
        if (maxUpdatesFromSource > MANGA_PER_SOURCE_QUEUE_WARNING_THRESHOLD && preferences.showLibraryUpdateThresholdWarning().get()) {
    ...
    suspend fun updateChapterList() {
        ...
        if (failedUpdates.isNotEmpty()) {
            writeErrors(failedUpdates)
            if (preferences.showLibraryUpdateErrors().get()) {
                notifier.showUpdateErrorNotification(
                    failedUpdates.size,
                )
            }
        } else {
            db.deleteLibraryErrors().executeAsBlocking()
        }
        // TX<--
     ...
     */
    // TX-->
    private fun writeErrors(errors: List<Pair<Manga, String?>>) {
        db.deleteLibraryErrors().executeAsBlocking()
        val errorItems = mutableListOf<LibraryUpdateError>()
        if (errors.isNotEmpty()) {
            errors.groupBy({ it.second }, { it.first }).forEach { (error, mangas) ->
                mangas.forEach {
                    errorItems.add(LibraryUpdateError.create(it.id!!, error!!))
                }
            }
        }
        db.insertLibraryErrors(errorItems).executeAsBlocking()
    }
    // TX<--
