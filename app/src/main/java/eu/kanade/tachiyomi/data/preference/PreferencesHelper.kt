...
class PreferencesHelper(val context: Context) {
    ...
    // original = false
    fun confirmExit() = prefs.getBoolean(Keys.confirmExit, true)

    // TX-->
    fun cacheSize() = flowPrefs.getString("eh_cache_size", "75")

    fun preloadSize() = flowPrefs.getInt("eh_preload_size", 4)

    fun readerThreads() = flowPrefs.getInt("eh_reader_threads", 3)

    fun aggressivePageLoading() = flowPrefs.getBoolean("eh_aggressive_page_loading", true)
    ...
    fun useAnimatedLoader() = flowPrefs.getBoolean("X_use_animated_loader", false)
    ...
    fun readerBottomButtons() = flowPrefs.getString("reader_bottom_buttons", ReaderBottomButton.BUTTONS_DEFAULTS)

    fun showLibraryUpdateThresholdWarning() = flowPrefs.getBoolean("X_show_library_update_warning", false)

    fun showLibraryUpdateErrors() = flowPrefs.getBoolean("X_show_library_update_errors", true)

    fun hideReaderMenuOnScroll() = flowPrefs.getBoolean("X_hide_reader_menu_on_scroll", false)

    fun libraryUpdateEndTIme() = flowPrefs.getLong("library_update_end_time", 0)
    ...
    fun showUpdateTimeSnack() = flowPrefs.getBoolean("show_update_time_snack", true)

    fun autoBackupTime() = flowPrefs.getLong("auto_backup_time", 0)

    fun autoBackupStatus() = flowPrefs.getString("auto_backup_status", "")
    // TX<--

    // original = true
    fun hideBottomBarOnScroll() = flowPrefs.getBoolean("pref_hide_bottom_bar_on_scroll", false)
    ...
    // original = true
    fun keepScreenOn() = flowPrefs.getBoolean("pref_keep_screen_on_key", false)
    ...
    // original = 0
    fun portraitColumns() = flowPrefs.getInt("pref_library_columns_portrait_key", 3)

    // original = 0
    fun landscapeColumns() = flowPrefs.getInt("pref_library_columns_landscape_key", 6)
    ...
    // original = 24
    fun libraryUpdateInterval() = flowPrefs.getInt("pref_library_update_interval_key", 5)

    // original = setOf(DEVICE_ONLY_ON_WIFI)
    fun libraryUpdateDeviceRestriction() = flowPrefs.getStringSet("library_update_restriction", emptySet())

    // original = setOf(MANGA_FULLY_READ, MANGA_ONGOING)
    fun libraryUpdateMangaRestriction() = flowPrefs.getStringSet("library_update_manga_restriction", emptySet())
    ...
    // original = DisplayModeSetting.COMPACT_GRID
    fun libraryDisplayMode() = flowPrefs.getEnum("pref_display_mode_library", DisplayModeSetting.COMFORTABLE_GRID)

    // original = false
    fun downloadBadge() = flowPrefs.getBoolean("display_download_badge", true)
    ...
    // original = false
    fun categoryNumberOfItems() = flowPrefs.getBoolean("display_number_of_items", true)
