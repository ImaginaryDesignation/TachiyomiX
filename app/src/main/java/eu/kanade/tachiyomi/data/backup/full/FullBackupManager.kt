...
class FullBackupManager(context: Context) : AbstractBackupManager(context) {
        ...
        try {
            file = (
                if (isAutoBackup) {
                    ...
                    // TX-->
                    val fName = context.resources.getString(R.string.app_name)
                    val backupRegex = Regex("""${fName}_\d+-\d+-\d+_\d+-\d+.proto.gz""")
                    // TX<--
                    ...
                    dir.createFile(BackupFull.getDefaultFilename(fName))
                } else {
            ...
            // TX-->
            preferences.autoBackupTime().set(System.currentTimeMillis())
            preferences.autoBackupStatus().set("Success")
            // TX<--

            return fileUri.toString()
        } catch (e: Exception) {
            // TX-->
            preferences.autoBackupStatus().set("Failed")
            // TX<--
    ...
    internal fun restoreCategories(backupCategories: List<BackupCategory>) {
        ...
        backupCategories.map { it.getCategoryImpl() }.forEach { category ->
            ...
            for (dbCategory in dbCategories) {
                ...
                if (category.name.equals(dbCategory.name, true)) {
                    // TX<--
        ...
        // TX-->
        val dbCats = databaseHelper.getCategories().executeAsBlocking()
        var order: Int = if (backupCategories[0].name != "All") {
            1
        } else {
            0
        }
        for (i in order until dbCats.size) {
            dbCats[i].order = order
            order += 1
        }
        databaseHelper.insertCategories(dbCats).executeAsBlocking()
        // TX<--
    }
    ...
    internal fun restoreCategoriesForManga(manga: Manga, categories: List<Int>, backupCategories: List<BackupCategory>) {
        ...
        // TX-->
        if (mangaCategoriesToUpdate.find { it.category_id == 1 } == null) {
            mangaCategoriesToUpdate += MangaCategory.create(manga, dbCategories[0])
        }
        // TX<--
