...
class ReaderPresenter(
    ...
    private fun isWebtoon(tags: List<String>, sourceName: String?): Int? {
        return when {
    ...
    fun getMangaReadingMode(resolveDefault: Boolean = true): Int {
        ...
        return when {
            resolveDefault && readingMode == ReadingModeType.DEFAULT && preferences.useAutoWebtoon().get() -> {
                isWebtoon(currentTags, sourceManager.get(manga.source)?.name) ?: default
            }
    ...
    fun saveImages(firstPage: ReaderPage, secondPage: ReaderPage, isLTR: Boolean, @ColorInt bg: Int) {
        ...
        val stream = mergeImages(firstPage, secondPage, isLTR, bg)

        val chapter = firstPage.chapter.chapter

        // Build destination file.
        val filenameSuffix = " - ${firstPage.number}-${secondPage.number}.jpg"
    ...
    private fun mergeImages(page1: ReaderPage, page2: ReaderPage, isLTR: Boolean, @ColorInt bg: Int): ByteArrayInputStream {
