...
import eu.kanade.tachiyomi.ui.recent.errors.LibraryUpdateErrorsController
...
class MainActivity : BaseActivity() {
    ...
    // TX-->
    private var lastNext: Boolean = false

    private var updateTimeSnack: Snackbar? = null
    // TX<--
        ...
        nav.setOnItemSelectedListener { item ->
            ...
            if (currentRoot?.tag()?.toIntOrNull() != id) {
                ...
                when (id) {
                    ...
                    R.id.nav_history -> {
                        router.setRoot(HistoryController(), id)
                        if (preferences.showUpdateTimeSnack().get()) {
                            updateTimeSnack = Snackbar.make(binding.rootCoordinator, updateTimeString(), 3000)
                            updateTimeSnack!!.setAction("Dismiss") {
                                updateTimeSnack!!.dismiss()
                            }
                            updateTimeSnack!!.anchorView = binding.bottomNav
                            updateTimeSnack!!.show()
                        }
                    }
                    // TX<--
                ...
                when (id) {
                    ...
                    R.id.nav_updates -> {
                        if (router.backstackSize == 1) {
                            router.pushController(LibraryUpdateErrorsController().withFadeTransaction())
                        }
                    }
                    R.id.nav_history -> {
                        if (preferences.showUpdateTimeSnack().get()) {
    ...
    private fun updateTimeString(): String {
        val interval = preferences.libraryUpdateInterval().get()
        val endTime = preferences.libraryUpdateEndTIme().get()
        ...
        var toastString = "Last update : Never"

        // Check if library has been updated before
        if (endTime != 0L) {
            // Check if library update is manual
            if (interval == 0 || lastNext) {
                toastString = "Last update : " + SimpleDateFormat(templateString).format(endTime) + "\n"
                ...
                toastString = "Next update : " + SimpleDateFormat(templateString).format(nextTime) + "\n"
    ...
    private fun syncActivityViewWithController(
        ...
        // TX-->
        if (to is LibraryUpdateErrorsController) {
            showNav(false)
        }
        // TX<--
