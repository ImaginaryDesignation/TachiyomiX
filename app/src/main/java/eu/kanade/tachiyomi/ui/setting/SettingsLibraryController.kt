    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) = screen.apply {
        ...
        preferenceCategory {
            ...
            preference {
                ...
                // TX-->
                fun getColumnValue(value: Int, portrait: Boolean): String {
                    return if (portrait && value == 3) {
                        value.toString() + " (" + context.getString(R.string.label_default) + ")"
                    } else if (!portrait && value == 6) {
                        value.toString() + " (" + context.getString(R.string.label_default) + ")"
                    } else {
                        value.toString()
                    }
                ...
                combine(preferences.portraitColumns().asFlow(), preferences.landscapeColumns().asFlow()) { portraitCols, landscapeCols -> Pair(portraitCols, landscapeCols) }
                    .onEach { (portraitCols, landscapeCols) ->
                        // TX-->
                        val portrait = getColumnValue(portraitCols, true)
                        val landscape = getColumnValue(landscapeCols, false)
                        // TX<--
        ...
        preferenceCategory {
            ...
            preference {
                ...
                // TX-->
                val catCount = dbCategories.size - 1
                // TX<--
        ...
        preferenceCategory {
            ...
            intListPreference {
                ...
                entryValues = arrayOf("0", "4", "5", "6", "8", "12", "24", "48", "72", "168")
                // TX<--
                summary = "%s"
            ...
            preference {
                ...
                fun updateSummary() {
                    ...
                    val allExcluded = excludedCategories.size == dbCategories.size

                    val includedItemsText = when {
                        // Some selected, but not all
                        includedCategories.isNotEmpty() && includedCategories.size != dbCategories.size -> includedCategories.joinToString { it.name }
                        // All explicitly selected
                        includedCategories.size == dbCategories.size -> context.getString(R.string.all)
            ...
            // TX-->
            switchPreference {
                key = Keys.showLibraryUpdateWarning
                titleRes = R.string.pref_show_library_update_warning
                summaryRes = R.string.pref_show_library_update_warning_summary
                defaultValue = false
            }
            switchPreference {
                key = Keys.showLibraryUpdateErrors
                titleRes = R.string.pref_show_library_update_error_notif
                defaultValue = true
            }
    ...
    class LibraryColumnsDialog : DialogController() {
        ...
        fun onViewCreated(binding: PrefLibraryColumnsBinding) {
            with(binding.portraitColumns) {
                // displayedValues = arrayOf(context.getString(R.string.label_default)) + IntRange(1, 10).map(Int::toString)
                // TX-->
                displayedValues = IntRange(1, 10).map {
                    if (it == 3) {
                        "$it (Default)"
                    } else {
                        it.toString()
