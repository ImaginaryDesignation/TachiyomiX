...
class PagerPageHolder(
    ...
    // TX-->
    /**
     * Subscription for status changes of second page.
     */
    private var extraStatusSubscription: Subscription? = null

    /**
     * Subscription for progress changes of second page.
     */
    ...
    private fun observeProgress() {
        ...
        progressSubscription = Observable.interval(100, TimeUnit.MILLISECONDS)
            .map { page.loadedBytes }
            .distinctUntilChanged()
            .onBackpressureLatest()
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe { value -> progressIndicator.updateProgress(value, page.totBytes, page.progress) }
    }
    // TX<--
    ...
    private fun observeProgress2() {
        ...
        extraProgressSubscription = Observable.interval(100, TimeUnit.MILLISECONDS)
            .map { extraPage.loadedBytes }
            .distinctUntilChanged()
            .onBackpressureLatest()
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe { value ->
                extraProgress = extraPage.progress
                progressIndicator.updateExtraProgress(value, extraPage.totBytes, extraProgress)
                // progressIndicator.updateProgress((value + page.loadedBytes), (page.totBytes + extraPage.totBytes), (progress + extraProgress) / 2 - 5)
            }
    }
    // TX<--
    ...
    private fun setImage() {
        ...
        // TX-->
        if (extraPage == null) {
            progressIndicator.updateProgress(0, 0, 0)
        } else {
            progressIndicator.hideExtra()
            progressIndicator.updateProgress(0, 0, 95, merge = true)
    ...
    private fun mergePages(imageStream: InputStream, imageStream2: InputStream?): InputStream {
        ...
        viewer.scope.launchUI {
            progressIndicator.updateProgress(0, 0, 96, merge = true)
            // progressIndicator.setProgress(96)
        }
        ...
        viewer.scope.launchUI {
            progressIndicator.updateProgress(0, 0, 97, merge = true)
            // progressIndicator.setProgress(97)
        }
        ...
        return ImageUtil.mergeBitmaps(imageBitmap, imageBitmap2, isLTR, viewer.config.pageCanvasColor) {
            viewer.scope.launchUI {
                if (it == 100) {
                    ...
                    progressIndicator.updateProgress(0, 0, it, merge = true)
