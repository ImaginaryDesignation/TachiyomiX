...
class PreMigrationController(bundle: Bundle? = null) :
    ...
    override fun getTitle() = view?.context?.getString(R.string.select_sources) + " (" + adapter?.items?.size.toString() + ")"
    // TX<--
    ...
    override fun onViewCreated(view: View) {
        ...
        // TX-->
        if (disIndex == -1) {
            for (i in 0 until ourAdapter.items.size) {
                if (!ourAdapter.items[i].sourceEnabled) {
                    disIndex = i
                    break
                }
            }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_select_all, R.id.action_select_none -> {
                ...
                // TX<--
                disIndex = if (item.itemId == R.id.action_select_all) {
                    adapter?.itemCount ?: 0
                } else {
                    0
                }
            ...
            R.id.action_match_enabled, R.id.action_match_pinned -> {
                ...
                // TX-->
                disIndex = if (item.itemId == R.id.action_match_enabled) {
                    adapter?.itemCount?.minus(enabledSources.size) ?: 0
                } else {
                    enabledSources.size
                }
                // TX<--
