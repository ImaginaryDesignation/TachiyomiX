...
class UpdatesController :
    ...
    override fun onViewCreated(view: View) {
        ...
        val simpleItemTouchCallback: ItemTouchHelper.SimpleCallback = object :
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                val item = adapter?.getItem(pos) as? UpdatesItem ?: return
                when (direction) {
                    ItemTouchHelper.LEFT -> toggleRead(item, pos)
                    ItemTouchHelper.RIGHT -> toggleBookmark(item, pos)
                }
            }
            ...
            override fun onChildDraw(
                ...
                val pos = viewHolder.absoluteAdapterPosition
                val item = adapter?.getItem(pos) as? UpdatesItem
                if (actionMode != null || item == null) {
                    ...
                    if (isCurrentlyActive) {
                        leftInt = if (item.read) {
                            R.drawable.ic_eye_off_24dp
                        } else {
                            R.drawable.ic_eye_24dp
                        }
                        rightInt = if (item.bookmark) {
    ...
    // TX-->
    private fun toggleRead(item: UpdatesItem, position: Int) {
        presenter.toggleChapterRead(item, !item.chapter.read)
        adapter?.notifyItemChanged(position)
    }

    private fun toggleBookmark(item: UpdatesItem, position: Int) {
        presenter.toggleChapterBookmark(item, !item.chapter.bookmark)
        adapter?.notifyItemChanged(position)
    }

    fun onChapterDeleted(item: UpdatesItem) {
        adapter?.updateItem(item, item)
    }
    // TX<--
