    ...
    fun fetchChaptersFromSource(manualFetch: Boolean = false) {
        ...
        fetchChaptersJob = presenterScope.launchIO {
            try {
                ...
                val (newChapters, removedChapters) = syncChaptersWithSource(db, chapters, manga, source)
                // TX<--
                if (manualFetch) {
                ...
                withUIContext { view?.onFetchChaptersDone(newChapters.size, removedChapters.size) }
                // TX<--
            } catch (e: Throwable) {
