...
import eu.kanade.tachiyomi.ui.manga.chapter.*
...
class MangaController :
    ...
    override fun onViewCreated(view: View) {
        ...
        val simpleItemTouchCallback: ItemTouchHelper.SimpleCallback = object :
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, swipeDir: Int) {
                if (viewHolder !is ChapterHolder || actionMode != null) return
                val pos = viewHolder.bindingAdapterPosition
                when (swipeDir) {
                    ItemTouchHelper.LEFT -> toggleMarkAsRead(pos)
                    ItemTouchHelper.RIGHT -> toggleBookmark(pos)
                }
            }
            ...
            override fun onChildDraw(
                ...
                if (viewHolder !is ChapterHolder || actionMode != null) {
                    super.onChildDraw(
                        c,
                        recyclerView,
                        viewHolder,
                    ...
                    val pos = viewHolder.bindingAdapterPosition
                    val item = chaptersAdapter?.getItem(pos)
                    val builder = RecyclerViewSwipeDecorator.Builder(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive)
                    builder.addSwipeLeftBackgroundColor(Color.parseColor("#7ADB8F"))
                    builder.addSwipeRightBackgroundColor(Color.parseColor("#AEC6FF"))
                    if (isCurrentlyActive) {
                        leftInt = if (item?.read == true) {
                            R.drawable.ic_eye_off_24dp
                        } else {
                            R.drawable.ic_eye_24dp
                        }
                        rightInt = if (item?.bookmark == true) {
                            R.drawable.ic_bookmark_border_24dp
                        } else {
                            R.drawable.ic_bookmark_24dp
                        }
                    }
                    builder.addSwipeLeftActionIcon(leftInt)
                    builder.addSwipeRightActionIcon(rightInt)
                    builder.create()
                        .decorate()
                    super.onChildDraw(
                        c,
        ...
        itemTouchHelper.attachToRecyclerView(binding.fullRecycler)
        // TX<--
    ...
    private fun updateToolbarTitleAlpha(@FloatRange(from = 0.0, to = 1.0) alpha: Float? = null) {
        ...
        // TX-->
        (activity as? MainActivity)?.binding?.appbar?.background?.alpha = when {
    ...
    private fun addToLibrary(manga: Manga) {
        ...
        when {
            // Default category is not All
            defaultCategoryId > 1 -> {
                toggleFavorite()
                val cats = categories.filter { it.id == 1 || it.id == defaultCategoryId }
                presenter.moveMangaToCategories(manga, cats)
            ...
            defaultCategoryId == 1 -> {
                toggleFavorite()
                presenter.moveMangaToCategory(manga, categories[0])
                activity?.toast(activity?.getString(R.string.manga_added_library))
                activity?.invalidateOptionsMenu()
            }
    ...
    fun onFetchChaptersDone(newChapterCount: Int, removedChapterCount: Int) {
        isRefreshingChapters = false
        updateRefreshing()
        addSnackbar?.dismiss()
        val snackString = if (removedChapterCount > 0) {
            "$newChapterCount new chapters ($removedChapterCount removed)"
        } else {
            "$newChapterCount new chapters"
        }
        addSnackbar = (activity as? MainActivity)?.binding?.rootCoordinator?.snack(snackString, 3000) {
            setAction("Dismiss") {
                this.dismiss()
            }
        }
    ...
    override fun onPrepareActionToolbar(toolbar: ActionModeWithToolbar, menu: Menu) {
        ...
        // TX-->
        toolbar.findToolbarItem(R.id.action_mark_previous_as_unread)?.isVisible = chapters.size == 1
        // TX<--
    }
    ...
    override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            // TX-->
            R.id.action_mark_previous_as_unread -> markPreviousAsUnRead(getSelectedChapters())
            // TX<--
            else -> return false
        }
        return true
    ...
    // TX-->
    private fun markPreviousAsUnRead(chapters: List<ChapterItem>) {
        val adapter = chaptersAdapter ?: return
        val prevChapters = if (presenter.sortDescending()) adapter.items.reversed() else adapter.items
        val chapterPos = prevChapters.indexOf(chapters.lastOrNull())
        if (chapterPos != -1) {
            markAsUnread(prevChapters.take(chapterPos))
        }
        destroyActionModeIfNeeded()
    }

    private fun toggleMarkAsRead(position: Int) {
        val item = chaptersAdapter?.getItem(position) ?: return
        presenter.markChaptersRead(listOf(item), !item.chapter.read)
        chaptersAdapter?.notifyItemChanged(position)
    }

    private fun toggleBookmark(position: Int) {
        val item = chaptersAdapter?.getItem(position) ?: return
